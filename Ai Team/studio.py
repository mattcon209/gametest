...
def write_launchers(primary_ext, run_cmd):
    """FIX (Audit12): write run.bat / run.sh for the detected language.

    These used to live at the BOTTOM of ensure_build_runnable(), which returns
    early whenever the integrator already produced a good main.* - i.e. on the
    SUCCESS path. Net effect: launchers only appeared when the fallback
    placeholder was written, exactly backwards. README promises
    "main.<ext> + run.bat/run.sh + README + DONE" as the finish, and for any
    compiled language (C++/C#/Rust) the missing run.bat means the user has no
    build command at all - the GUI 'Run Build' button just opens the folder.
    Split out so BOTH paths call it.
    """
    BUILD_DIR.mkdir(exist_ok=True)
    cmds = {
        ".py":  "python main.py",
        ".cpp": "g++ main.cpp -o game.exe && game.exe",
        ".cs":  "dotnet run",
        ".lua": "lua main.lua",
        ".gd":  "echo Open this folder as a project in Godot",
        ".rs":  "cargo run",
        ".js":  "node main.js",
        ".ts":  "npx ts-node main.ts",
    }
    win_cmd = cmds.get(primary_ext, run_cmd)
    (BUILD_DIR / "run.bat").write_text(f"@echo off\n{win_cmd}\npause\n", encoding="utf-8")

    # run.sh mirrors it, minus the .exe-ism on the C++ line
    sh_cmds = dict(cmds)
    sh_cmds[".cpp"] = "g++ main.cpp -o game && ./game"
    sh_cmd = sh_cmds.get(primary_ext, run_cmd)
    sh = BUILD_DIR / "run.sh"
    ...
    # FIX (Audit12): build/README.md is promised in the README's pipeline line
    # ("main.<ext> + run.bat/run.sh + README + DONE") but was never written by
    # any code path. Matters most for compiled languages, where the build
    # command is not guessable from the folder contents.
    (BUILD_DIR / "README.md").write_text(
        "# Build Output\n\n"
        f"Entry point: `main{primary_ext}`\n\n"
        "## Run it\n\n"
        f"- Windows: double-click `run.bat` (or: `{win_cmd}`)\n"
        f"- Linux/macOS: `./run.sh` (or: `{sh_cmd}`)\n\n"
        "## Notes\n\n"
        "- Generated by the Ai Team orchestrator by merging fragments from `output/`.\n"
        "- `test_results.txt` holds the smoke-test result from the build that produced this.\n"
        "- `DONE` marks a completed build. Delete it (or use the GUI's Force Rebuild) to rebuild.\n"
        "- Compiled languages need their toolchain on PATH (g++ / dotnet / cargo).\n",
        encoding="utf-8")
    return win_cmd
...
def ensure_build_runnable(primary_ext, run_cmd, engine_mode, gdd_text):
    ...
    if main_path.exists() and main_path.stat().st_size > 10:  # FIX N1: Lowered from 100 to 10, 44-char games valid
        # FIX (Audit12): the LLM produced a real main.* - still guarantee the
        # launchers exist. Previously this early return skipped them entirely.
        write_launchers(primary_ext, run_cmd)
        return main_path
...
''',
    ...
    # Launchers via the shared writer (see write_launchers docstring)
    write_launchers(primary_ext, run_cmd)
    return main_path
...
"""
            ...
            try:
                ...
                # FIX (Audit12) DUPLICATE TASK IDS - this was a hard run-breaker.
                # The old code was `if "id" not in nt:` i.e. it TRUSTED whatever id
                # the LLM returned. AURA's own prompt shows `{"id":1,...}` as the
                # example, so models routinely return 1,2,3 on every planning cycle.
                # Those collide with existing tasks and with the ids the diversity
                # injector hands out from next_id.
                # Impact: the parallel results loop and the validation loop both
                # locate a task with `if tt.get("id") == task_item.get("id"): ... break`
                # - which matches the FIRST task holding that id. A finished task's
                # result gets written onto an unrelated older task, while the real
                # one stays "in_progress" forever. pending never drains, so
                # `len(pending)==0` is never true and INTEGRATOR NEVER RUNS:
                # fragments accumulate in output/ but no build, no DONE, no export.
                # Reproduced on a C++ GDD: 4 fragments written, 2 tasks wedged at
                # in_progress, zero INTEGRATOR activity across 245s of running.
                # Fix: always assign ids ourselves; never trust the model's.
                _used_ids = {t.get("id") for t in tasks}
                for nt in new_tasks:
                    if nt.get("id") in _used_ids or not isinstance(nt.get("id"), int):
                        nt["id"] = next_id
                    while nt["id"] in _used_ids:
                        nt["id"] = next_id
                        next_id += 1
                    _used_ids.add(nt["id"])
                    next_id = max(next_id, nt["id"] + 1)
                    is_dup = False
                ...
                if missing_roles and len(filtered) < 5:
                    ...
                    # FIX (Audit12): allocate from the SAME collision-checked pool as
                    # the AURA-planned tasks above, otherwise these re-issue ids that
                    # the model already used (see the duplicate-id note above).
                    _used_ids |= {t.get("id") for t in filtered}
                    for mr in missing_roles[:2]:  # Add up to 2 missing roles
                        if mr not in [t.get("role") for t in filtered]:
                            while next_id in _used_ids:
                                next_id += 1
                            _used_ids.add(next_id)
                            filtered.append({
                                "id": next_id,
                                "role": mr,
